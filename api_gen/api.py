...
from api_gen.helpers import TYPE_CONVERTION, operation_id_to_function_name
...
class Api:
    ...
    def generate_base_imports(self, client_kind: Literal["sync", "async"] = "sync"):
        self.data.append("from api_mcp.server import mcp_server")
        if client_kind == "sync":
...
# Parse and dynamically create routes
# for path, methods in spec.get("paths", {}).items():
#     for method, operation in methods.items():
#         if method not in ["get", "post", "put", "delete", "patch"]:
#             logger.warning(
#                 f"Unsupported method {method} for path {path}. Skipping.")
#             continue

#         operation_id = operation.get("operationId")
#         if not operation_id:
#             logger.warning(
#                 f"Operation ID not found for {method} {path}. Skipping.")
#             continue

#         # Define an MCP-style command trigger
#         api_info["operations"][operation_id] = {
#             "path": path,
#             "method": method.upper(),
