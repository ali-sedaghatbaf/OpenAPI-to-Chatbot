...
def generate_api(input_path: str, output_path: str = "api_tools"):
    ...
    for tag in get_all_tags(yaml_data["paths"]):
        api = Api(
            yaml_data["paths"],
            base_url=yaml_data.get("servers", [{"url": "http://localhost:8080"}])[0][
                "url"
            ],
            only_tag=tag,
        )

        api.generate_apis(schema_path=output_path + ".schema", client_kind="async")  # type: ignore
