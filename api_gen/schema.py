...
class Schema:
    ...
    def create_attribute(self, class_name: str, component: dict):
        ...
        for name, type_info in component["properties"].items():
            is_optional = name not in component.get("required", [])
            ...
            match type_info.get("type", "reference"):
                case "string":
                    if type_info.get("maxLength") or type_info.get("minLength"):
                        ...
                        if format_ := type_info.get("format"):
                            ...
                            enum_name = self.create_enum(
                                f"{class_name}_{name}", enum_)
                            type_hint = enum_name
                        else:
                            type_hint = "str"
                case "integer":
                ...
                case "array":
                    ...
                    if reference := type_info["items"].get("$ref"):
                        ref = function_like_name_to_class_name(
                            reference.split("/")[-1])
            ...
            if is_optional and type_hint is not None:
                class_info["attributes"].append(
                    f"{name}{delimiter}Optional[{type_hint}] = None")
                if not type_info.get("nullable", False):
                    ...
                    class_info["validators"].append(
                        create_validator(name, field_type))
            elif not is_optional and type_hint is not None:
                class_info["attributes"].append(
                    f"{name}{delimiter}{type_hint}")
        return class_info
    ...
    def write_to_file(self, folder_path: Path, additional_data: list[str] = None):
        ...
        if additional_data:
            ...
            data.append("\n")
        data.extend([self.create_enum_class(obj)
                    for obj in self.enums.values()])
        data.append("\n")
        ...
        with schema_file.open("w", encoding="utf-8") as file:
            file.write(text)
            isort.api.sort_file(schema_file)
